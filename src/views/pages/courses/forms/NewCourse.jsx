...
export function BasicTabs() {
    ...
    const [value, setValue] = useState(
        tab ? ['general', 'description', 'course-format', 'section', 'appearance', 'completion', 'groups'].indexOf(tab) : 0
    );
    ...
    useEffect(() => {
        if (slug) {
            axios
                .get(`http://localhost:8080/api/courses/slug/${slug}`)
                .then((response) => {
                    setCourseData(response.data);
                    setCourseId(response.data._id);
                    setNoOfSection(Number(response.data.noOfSection) || 0);
                })
                .catch((error) => {
    ...
    const goToPreviousTab = () => {
        ...
        const updateFormData = (section, data) => {
            ...
            if (section === 'courseFormat') {
                setNoOfSection(data.noOfSection); // Update the section count when courseFormat is updated
            }
        };
    };
    return (
